...
featureData = readMat("/Users/kenjilee/Documents/GitHub/PhysMAP_Chand/CellExplorerv2/Data/features_cellExp_new_Nov2023.mat")
...
fullAllenData = setdiff(actualData, selectedData)
...
whichAssay = "features"
data = runAnalysis(data, whichAssay, RESOLUTION = RESOLUTION, normalize=NORMALIZE, numPCS=10, DimV=1:10)
...
whichAssay = "ISI1"
data = runAnalysis(data, whichAssay, RESOLUTION = RESOLUTION, normalize=NORMALIZE)
p1 = DimPlot(data, reduction = paste0(whichAssay, 'plotumap'), pt.size=2)
...
data <- FindMultiModalNeighbors(
  data, reduction.list = list("WFpca","ISI1pca","ACGpca"), 
  dims.list = list(1:40, 1:40,1:40),knn.range = 100
)
...
p33 <- DimPlot(data, reduction = 'wnn.umap', pt.size=2, label = FALSE, repel = TRUE, label.size = 8)
p33 <- p33 + theme_minimal()
show(p33)

p32 <- DimPlot(data, reduction = 'wnn.umap', pt.size=2, label = FALSE, repel = TRUE, label.size = 8, group.by="cType", cols = cValues)
